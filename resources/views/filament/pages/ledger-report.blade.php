    ...
    <section class="space-y-4">
        <form method="GET" class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm ring-1 ring-gray-950/5 dark:border-white/10 dark:bg-gray-900 dark:ring-white/10">
            <div class="grid gap-4 lg:grid-cols-12">
                ...
                <label class="space-y-1.5 lg:col-span-3">
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-200">Cuenta PUC</span>
                    <select name="chart_account_id" class="block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:border-white/10 dark:bg-white/5 dark:text-white">
                ...
                <div class="flex items-end gap-2 lg:col-span-2">
                    ...
                    <a href="{{ route('accounting-reports.ledger', array_merge(request()->query(), ['export' => 1])) }}" class="inline-flex h-10 items-center justify-center rounded-lg border border-gray-300 bg-white px-4 text-sm font-semibold text-gray-700 shadow-sm transition hover:bg-gray-50 dark:border-white/10 dark:bg-white/5 dark:text-gray-100 dark:hover:bg-white/10">
                        CSV
                    </a>
                </div>
            </div>
        ...
        <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm ring-1 ring-gray-950/5 dark:border-white/10 dark:bg-gray-900 dark:ring-white/10">
            <div class="border-b border-gray-200 px-4 py-3 dark:border-white/10">
                <p class="text-sm font-semibold text-gray-950 dark:text-white">Movimientos contables</p>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ number_format($entries->total()) }} registros encontrados</p>
            </div>

            <div class="overflow-x-auto">
                <table class="min-w-[1180px] w-full divide-y divide-gray-200 text-left text-sm dark:divide-white/10">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/10">
                        @forelse ($entries as $entry)
                            <tr class="transition hover:bg-gray-50 dark:hover:bg-white/5">
                                ...
                                <td class="px-4 py-3 text-gray-900 dark:text-white">{{ $entry->chartAccount->full_name }}</td>
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-200">{{ $entry->thirdParty?->name ?? 'Sin tercero' }}</td>
                                <td class="max-w-md px-4 py-3 text-gray-500 dark:text-gray-400">{{ $entry->description ?: 'Sin descripción' }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-12 text-center text-sm text-gray-500 dark:text-gray-400">No hay movimientos para los filtros seleccionados.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
